...
TESTING GUIDELINES
------------------					
There are 10 test cases written for Kernel #1 assignment. These can be executed by providing the test case number thorugh the variable 'curtest' in the 'kmain.c' file.
...
Description: The test case exits immidiately from the init_proc().
...
Description: We are testing proc_kill_all functionality. Here we are calling process_kill_all() from the leaf node in the hierarchy, which will kill all the processes under INIT Process cleanly and exit from the kernel graccefully.

Test Case 4:	KTHREAD_ CANCEL TEST
Description: We are calling kthread_cancel() on a specific process thread in the tree heirachy. The kernel should exit gracefully upon execution of the test case. The child processes of the deleted/killed process are re-parented under INIT process, if any.

Test Case 5:	KTHREAD_EXIT TEST
Description: We are calling kthread_exit() on currently running thread. The kernel should exit gracefully upon execution of the test case. The child processes of the deleted/killed process are re-parented under INIT process, if any.

Test Case 6:	PRODUCER-CONSUMER TEST
Description: We have defined two processes 'producer' and 'consumer'. The 'producer' process has 'produce_thread'. The 'consumer' process has 'consume_thread'.
The resource 'buffer' is commonly shared by both threads. There is only one queue 'special_q'. The 'produce_thread' puts one item in buffer, sleep in the queue and wakes up the 'consume_thread'. The 'consume_thread' removes the item from the buffer, sleep in the queue and wakes up the 'produce_thread'.


Test Case 7:	DEADLOCK TEST
Description: We have defined two processes 'DEADLOCK1' and 'DEADLOCK2'. The 'DEADLOCK1' process has 'dead1'. The 'DEADLOCK2' process has 'dead2'. The 'dead1' thread locks the mutex 'm1' and try to lock the mutex 'm2'. The 'dead2' thread locks the mutex 'm2' and try to lock the mutex 'm1'. As a result, both the threads are in the wait queuen and deadlock is done and console will be hung.
...
Description: In this test we are testing the deadloack condition on the thread itself. Here a thread who owns a mutex lock try to lock it again. This will cause an KASSERT error and the kernel will be halted w/o cleanly.
