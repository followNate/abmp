...
proc_cleanup(int status)
{
...
#ifdef __MTP__
	kthread_t *kthr;
	list_iterator_begin(&p_threads,kthr, kthread_t, kt_plink){
		list_remove_tail(kthr);
	}list_iterator_end();
#else
	list_remove_head(&p_threads);
#endif
	...
	proc_t *child;
	list_iterator_begin(&p_children,child,proc_t,p_child_link){
